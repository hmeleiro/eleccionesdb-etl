...
territorios <-
  ...
  select(territorio_id = id, codigo_ccaa, codigo_provincia,
...
# 2007
df <- read_excel(paste0(INPUT_DIR, "2007_Mesas.xls"))
...
remove_cols <- c("censo_total", "votos_electores", "votos_interventores",
...
df_pre95 <-
  map_df(ficheros, function(fichero) {
    ...
    read_excel(fichero, sheet = "PROVINCIAS (OFICIALES)") %>%
      janitor::clean_names() %>%
      select(-anyo) %>%
...
info_cer <-
  ...
  mutate(
    across(c(codigo_provincia, codigo_circunscripcion, codigo_distrito),
           ~ifelse(is.na(.), "99", .)),
