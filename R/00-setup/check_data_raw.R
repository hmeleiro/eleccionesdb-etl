# Script: check_data_raw.R
# Validates data-raw/ after restoring the GitHub Actions cache or downloading.
...
prepare_data_index <- function(index) {
    ...
    index <- index[!is.na(index$key) & nzchar(index$key), , drop = FALSE]
...
if (!dir_exists(data_dir)) {
    stop("No existe data-raw/.", call. = FALSE)
}
...
message("data-raw/ existe: si")
message("Ficheros esperados en manifiesto: ", nrow(index))
message("Ficheros presentes en data-raw/: ", length(files))
message("Tamano total de data-raw/: ", format_bytes(total_size))

if (!length(files) || total_size <= 0) {
    stop("data-raw/ esta vacio.", call. = FALSE)
}

message("data-raw/ verificado correctamente.")
