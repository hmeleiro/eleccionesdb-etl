...
#   run_fact_checks()
#   # or:
#   Rscript R/tests/validate_tablas_finales.R
#
...
#' Validate elecciones_fuentes dimension.
#' Checks structure (eleccion_id, fuente, url_fuente, observaciones),
#' uniqueness on eleccion_id, and warns (not errors) for missing fuentes.
#'
#' @examples
#' validate_dim_elecciones_fuentes(elecciones_fuentes_df, elecciones_df)
validate_dim_elecciones_fuentes <- function(df, elecciones_df = NULL,
                                            label = "elecciones_fuentes") {
  expected_cols <- c("eleccion_id", "fuente", "url_fuente", "observaciones")
  stop_if_not_all(
    setequal(names(df), expected_cols),
    paste0(
      "[", label, "] Columnas inesperadas o faltantes: ",
  ...
  stop_if_not_all(
    !duplicated(df$eleccion_id),
    paste0("[", label, "] $eleccion_id debe ser único (PK)")
  )

  # Avisar (no fallar) si hay filas sin fuente
  sin_fuente <- df %>% dplyr::filter(is.na(fuente) | is.na(url_fuente))
  if (nrow(sin_fuente) > 0) {
    warning(sprintf(
      "[%s] %d elecciones sin fuente documentada (eleccion_id: %s)",
      label, nrow(sin_fuente),
      paste(sin_fuente$eleccion_id, collapse = ", ")
    ))
  }

  # FK: eleccion_id -> elecciones$id (si se proporciona)
  if (!is.null(elecciones_df)) {
    bad_fk <- !(df$eleccion_id %in% elecciones_df$id)
    stop_if_not_all(
      !bad_fk,
      paste0("FK: ", label, "$eleccion_id debe existir en elecciones$id")
    )
  }
...
run_dimension_checks <- function(
  ...
  # elecciones_fuentes — optional: warn only if file exists or df provided
  fuentes_path <- "tablas-finales/dimensiones/elecciones_fuentes"
  if (!is.null(elecciones_fuentes) || file.exists(fuentes_path)) {
    fuentes_df <- .resolve_df(elecciones_fuentes,
      fuentes_path, readr::read_csv,
      show_col_types = FALSE
    )
    validate_dim_elecciones_fuentes(fuentes_df, elec_df)
  }
