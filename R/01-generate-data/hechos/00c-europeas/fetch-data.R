...
OUTPUT_DIR <- "data-raw/hechos/00c-europeas/"
dir.create(OUTPUT_DIR, showWarnings = FALSE, recursive = TRUE)

# --- Fechas de elecciones europeas ---
fechas <-
...
safe_mesas <- possibly(
    function(anno, mes) mesas(tipo_eleccion = "europeas", anno = anno, mes = mes),
    otherwise = tibble()
)

safe_provincias <- possibly(
    function(anno, mes) provincias(tipo_eleccion = "europeas", anno = anno, mes = mes),
    otherwise = tibble()
)

safe_municipios <- possibly(
    function(anno, mes) {
        municipios(tipo_eleccion = "europeas", anno = anno, mes = mes, distritos = FALSE)
    },
    otherwise = tibble()
)
