...
codigos_ccaa <- readRDS("data-raw/hechos/00-congreso/codigos_ccaa.rds") %>% select(-ccaa)
...
INPUT_DIR <- "data-raw/hechos/00-congreso/"

# Mesa level (finest granularity)
data_mesas <-
    readRDS(paste0(INPUT_DIR, "mesas.rds")) %>%
    mutate(
...
data_prov_raw <-
    readRDS(paste0(INPUT_DIR, "provincias.rds")) %>%
...
validate_votos_partido_match(votos, label = "00-congreso/votos")
